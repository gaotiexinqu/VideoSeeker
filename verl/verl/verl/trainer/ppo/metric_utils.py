...
def compute_reward_extra_metrics(batch: DataProto) -> dict[str, Any]:
    """
    Compute metrics from extra reward info stored in batch.non_tensor_batch.

    Extracts format_reward_score and acc_score (produced by custom reward functions
    that return a dict, e.g. score = 0.8 * acc_reward + 0.2 * format_reward) and
    computes mean / max / min statistics for wandb logging.

    Returns an empty dict if the keys are not present.
    """
    metrics = {}

    for key, metric_prefix in [
        ("format_reward_score", "reward/format_reward"),
        ("acc_score", "reward/acc_reward"),
    ]:
        ...
        try:
            ...
            metrics[f"{metric_prefix}/max"] = float(arr.max())
            metrics[f"{metric_prefix}/min"] = float(arr.min())
        except Exception:
            pass
