...
def _get_input_embeds(
    ...
    if pixel_values is not None:
        ...
        if n_image_tokens != n_image_features:
            ...
            print(
                f"\n{'=' * 80}\n"
                f"[DEBUG: image features and tokens mismatch]\n"
                f"  n_image_tokens: {n_image_tokens}\n"
                f"  n_image_features: {n_image_features}\n"
                f"  diff (features - tokens): {n_image_features - n_image_tokens}\n"
                f"  image_grid_thw:\n",
                end="", file=sys.stderr
            )
            if image_grid_thw is not None:
                for idx, row in enumerate(image_grid_thw):
                    t, h, w = row[0].item(), row[1].item(), row[2].item()
                    print(f"    image[{idx}]: T={t}, H={h}, W={w}, patches={t * h * w}", file=sys.stderr)
            else:
                print(f"    image_grid_thw is None", file=sys.stderr)
            print(
                f"  pixel_values.shape: {pixel_values.shape if pixel_values is not None else None}\n"
                f"  raw_patches (pixel_values.shape[0]): {pixel_values.shape[0] if pixel_values is not None else None}\n"
                f"  image_grid_thw.prod().sum(): {image_grid_thw.prod().sum().item() if image_grid_thw is not None else None}\n"
                f"  input_ids.shape: {input_ids.shape}\n",
                file=sys.stderr
            )
            img_token_pos = (input_ids == model.config.image_token_id).nonzero(as_tuple=True)[0]
            print(
                f"  image_token positions (first 10): {img_token_pos[:10].tolist()}\n"
                f"  num_image_tokens in input_ids: {len(img_token_pos)}\n"
                f"{'=' * 80}\n",
                file=sys.stderr
            )
            raise ValueError(
    ...
    if pixel_values_videos is not None:
        ...
        if n_video_tokens != n_video_features:
            ...
            print(
                f"\n{'=' * 80}\n"
                f"[DEBUG: video features and tokens mismatch]\n"
                f"  n_video_tokens: {n_video_tokens}\n"
                f"  n_video_features: {n_video_features}\n"
                f"  video_grid_thw:\n",
                end="", file=sys.stderr
            )
            if video_grid_thw is not None:
                for idx, row in enumerate(video_grid_thw):
                    t, h, w = row[0].item(), row[1].item(), row[2].item()
                    print(f"    video[{idx}]: T={t}, H={h}, W={w}, patches={t * h * w}", file=sys.stderr)
            else:
                print(f"    video_grid_thw is None", file=sys.stderr)
            print(
                f"  pixel_values_videos.shape: {pixel_values_videos.shape if pixel_values_videos is not None else None}\n"
                f"  raw_patches (pixel_values_videos.shape[0]): {pixel_values_videos.shape[0] if pixel_values_videos is not None else None}\n"
                f"  video_grid_thw.prod().sum(): {video_grid_thw.prod().sum().item() if video_grid_thw is not None else None}\n"
                f"{'=' * 80}\n",
                file=sys.stderr
            )
            raise ValueError(
