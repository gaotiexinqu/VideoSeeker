...
def initialize_tools_from_config(tools_config_file):
    ...
    def get_mcp_event_loop():
        ...
        if tmp_event_loop is None:
            ...
            thread = threading.Thread(target=run_loop, args=(tmp_event_loop,), name="mcp tool list fetcher", daemon=True)
            thread.start()
    ...
    try:
        ...
        if tmp_event_loop is not None:
            # Cancel all pending tasks first, then stop the loop
            def cancel_all_tasks():
                for task in asyncio.all_tasks(tmp_event_loop):
                    task.cancel()
            tmp_event_loop.call_soon_threadsafe(cancel_all_tasks)
            tmp_event_loop.call_soon_threadsafe(tmp_event_loop.stop)
            if thread is not None and thread.is_alive():
                thread.join(timeout=10.0)
            # Only close if the loop is no longer running
            if not tmp_event_loop.is_running():
